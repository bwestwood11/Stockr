...
useEffect(() => {
  const getStocks = async () => {
    if( session?.user?.id) {
    const response = await fetch(`/api/getstocks/${session?.user.id}/stocks`);
    const data = await response.json();
    console.log(data);
    setStocks(data);
}
...
const handleDelete = async (id) => {
  const hasConfirmed = confirm('Are you sure you want to remove this stock from your watch list?');
